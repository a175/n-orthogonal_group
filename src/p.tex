...
内積は $\Braket{\bullet,\bullet}$で表すとする.
つまり,
\begin{align*}
  V&=\RR^n\\
...
である.
直接計算することで,
$A_\sigma \ee_k=\ee_{\sigma(k)}$であることわかる.
...
\begin{proof}
  ...
  $A_{\sigma}$が鏡映であるとする.
  このとき, $A_{\sigma}^2=E_n$であるので,
  $\sigma^2=\varepsilon$である.
  したがって, $\sigma=(i_1j_1)\cdots(i_l,j_j)$
  と文字を共有しない互換の積としてかける.
  $l=1$というのは$\sigma$が互換であることを意味する.
  $l\geq 2$を仮定する.
  $\alpha=\ee_{i_1}-\ee_{j_1}$,
  $\beta=\ee_{i_2}-\ee_{j_2}$
  とおく.
  $A_{\sigma}\alpha=-\alpha$である.
  一方, $\beta\in H_\alpha$であるが,
  $A_{\sigma}\beta=-\beta$であるので, これは鏡映ではない.
