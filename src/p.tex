...
\begin{prop}
  ...
  $\bar f(\bar\alpha)=\overline{f(\alpha)}$とし,
  $\bar f\colon V\to V'$を$\bar f(\sum_{\delta\in\bar\Delta}c_\delta \delta)=\sum_{\delta\in\bar\Delta}c_\delta \bar f(\delta)$
  で定義する.
  任意の$\alpha,\beta\in \bar\Phi$に対し$\Braket{f(\alpha),f(\beta)}=\Braket{\alpha,\beta}$となることを仮定する.
