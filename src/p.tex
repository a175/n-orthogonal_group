...
$i<j$に対し,
\begin{align*}
T_{i,i+1}T_{i+1,i+2}\cdots T_{n-2,n-1} \alpha_{n-1,n}=\alpha_{i,n}\\
T_{j,j+1}T_{j+1,j+2}\cdots T_{n-1,n} \alpha_{i,n}=\alpha_{i,j}
\end{align*}
である. つまり, $\alpha_{i,j}$に対し,
$w(\alpha_{n-1,n})=\alpha_{i,j}$をみたす
$w\in \Braket{S}_\ZZ$がとれる.
したがって,
\cref{prop:conjbyortho,prop:reflgrpisortho}
から,
\begin{align*}
T_{i,j}=s_{\alpha{i,j}}=s_{w(\alpha{n-1,n})}=ws_{\alpha_{n-1,n}}w^{-1}=wT_{n-1,n}w^{-1}
\end{align*}
