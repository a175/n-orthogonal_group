...
\begin{prop}
  $V$を$n$次元ユークリッド空間,
  $U$を$m$次元ユークリッド空間とする.
  線形写像$t\colon V\to W$は内積を保つとする.
  $\Phi\subset V$, $\Psi\subset W$はそれぞれルート系であるとし,
  $f\colon \Phi \to \Psi$は全単射であるとする.
  各$\alpha\in\Phi$に対し,
  $f(\alpha)= c_\alpha t(\alpha)$をみたす$c_\alpha>0$が存在するとする.
  このとき, $W(\Phi)$と$W(\Psi)$は群として同型である.
\end{prop}
\begin{proof}
  $\hat U=\Set{t(\alpha)|\alpha\in V}$とおき, $\check U=\hat U^\perp$とする.
  このとき, $U=\hat U \oplus \check U$である.
  $\beta\in \Psi$に対し,
  $\alpha=f^{-1}(\beta)\in \Phi\subset V$とおくと,
  $c_{\alpha}t(\alpha)=f(\alpha)=\beta$であるので,
  $\Psi\subset \hat U$である.
