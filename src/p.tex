...
\end{align*}
とする. これは$V$の部分空間となる.
$U\cap W=\emptyset$かつ$V=U+W$
となるとき,
$V$は$U$と$W$の(内部)直和に分解されるといい,
$V=U\oplus V$とかく.

$U\subset V$に対し,
\begin{align*}
  U^\perp=\Set{v\in V| \forall u\in U,\Braket{u,v}=0}
\end{align*}
とする. これは$V$の部分空間になる.
これを$U$の直交補空間とよぶ.
$V$の部分空間$U$に対し,
$V=U\oplus U^\perp$である.
...
は$t$の多項式であるが,
これを$A$の固有多項式とよぶ.
$Av=\lambda v$をみたす$\lambda\in \RR$, $v\in V\setminus\Set{\zzero}$
があったとき,
$v$のことを
固有値$\lambda$に属する固有ベクトルとよぶ.
固有値$\lambda$に属する固有ベクトルを集めた集合と$\Set{\zzero}$の和集合は,
ベクトル空間になる.
これを固有値$\lambda$に属する固有空間と呼ぶ.
...
%$\transposed{A}=A$を満たすとき$A$を対称行列とよぶ.
...
$V$と$W$をそれぞれベクトル空間とする.
写像$f\colon V\to W$が次の条件を満たすとき,
$f$は線形写像であるという:
\begin{align*}
  v,w\in V &\implies f(v+w)=f(v)+f(w)\\
  v\in V,c\in \RR &\implies f(cv)=cf(v)  .
\end{align*}
...
$(m,n)$-行列$A$が与えられたとする.
このとき, $v\in V$に対し, $f_A(v)=Av$と定義すると$f_A(v)\in W$である.
...
\begin{definition}
  ...
  $\emptyset \neq S\subset G$に対し,
  \begin{align*}
    \Braket{S}_{\ZZ}=\Set{g_1^{i_1}\cdots g_n^{i_n}|n=1,\ldots;i_j\in \ZZ;g_i\in G}
  \end{align*}
  とする.
  ただし,
...
\begin{align*}
  ...
  P_{2\theta}(c\alpha(\theta+\frac{\pi}{2})) &= c\alpha(\theta+\frac{\pi}{2})
\end{align*}
...
\item
  ...
  \begin{enumerate}
  \item
    $R_\theta R_\tau=R_{\theta+\tau}$.
  \item
    $P_\theta P_\tau=R_{\theta-\tau}$.
  \item
    $P_\theta R_\tau=P_{\theta-\tau}$.
  \item
    $R_\theta P_\tau=P_{\theta-\tau}$.
  \end{enumerate}
\item
...
\item 
  ...
  とする. $k\in\ZZ$に対し次を示せ:
  \begin{enumerate}
    \item $P_{\frac{2k\pi}{m}} \alpha\left(\frac{i\pi}{m}\right)\in S$.
    \item $R_{\frac{2k\pi}{m}} \alpha\left(\frac{i\pi}{m}\right)\in S$.
  \end{enumerate}
\item
  $S$を次の集合とする.
  \begin{align*}
  ...
  $i\neq j$に対し次を示せ:
  \begin{enumerate}
    \item $T_{1,2}(\ee_i-\ee_j) \in S$.
    \item $T_{2,3}(\ee_i-\ee_j) \in S$.
  \end{enumerate}
\end{enumerate}
...
\begin{prop}
  ...
  $t\colon V\to V'$を$\bar t(\sum_{\delta\in\bar\Delta}c_\delta \delta)=\sum_{\delta\in\bar\Delta}c_\delta t(\delta)$
  で定義する.
  任意の$\alpha,\beta\in \bar\Phi$に対し$\Braket{t(\alpha),t(\beta)}=\Braket{\alpha,\beta}$となることを仮定する.
  また, $\Set{\bar f(\alpha)|\alpha\in\bar\Phi}=\bar\Phi'$も仮定する.
  このとき, $W(\Phi)$と$W(\Phi')$は群として同型である.
\end{prop}
\begin{proof}
  $\hat U=\Set{t(\alpha)|\alpha\in V}$とおき, $\check U=\hat U^\perp$とする.
  このとき, $U=\hat U \oplus \check U$である.
  $\Phi'\subset \hat U$である.
  
  \Cref{lem:subsp:refl}より,
  $\beta\in\bar\Phi'$に対し,
  $\hat s_\beta\colon \hat U \to \hat U$
  を$\hat s_\beta(\gamma)= s_\beta(\gamma)$で定義すると,
  $\beta$に関する$U$上の鏡映変換である.
  したがって$U$上の全単射である.
  また, $\gamma\in \check U$に対して, $s_\beta (\gamma)=\gamma$である.
  $w\in W(\Psi)$は,
  $\beta_i\in \Psi$を用いて,
  $w=s_{\beta_1}\cdots s_{\beta_l}$とかけるので,
  $\hat w\colon \check U\to \check U$
  を$\hat w(\gamma)= w(\gamma)$で定義すると,
  全単射である.
  また, $\gamma\in\check U$に対して, $w(\gamma)$である.

  $\hat W=\Set{\hat w|w\in W(\Psi)}$とおく.
  このとき,
  $g\colon W(\Phi) \to W'$を,
  $g(w)=\hat w$で定義すると,
  明らかに全射である.
  ...
  $\gamma\in \hat U$に対し,
  $w(\gamma)=v(\gamma)$である.
  また$\gamma\in \check U$に対し,
  $w(\gamma)=\gamma=v(\gamma)$である.
  したがって, $w=v$であるので, $g$は単射でもある.
  $g(wv)=g(w)g(v)$を満たすことは定義から明らかなので,
  $\hat W$は$W$と同型である.

  
  また, $\hat t\colon V \to \hat U$を$\hat t(\alpha)=t(\alpha)$で定義すると,
  $\hat t$は内積を保存し, 全単射である.
  $w\in W(\Phi)$に対し,
  $\hat t w \hat t^{-1}\colon \hat U \to \hat U$を,
  $h(w)$とおく.
  $w\in W(\Phi)$に対し,
  $w=s_{\alpha_1}\cdots s_{\alpha_l}$をみたす$\alpha_i\in\Phi$がとれるが,
  \cref{prop:conjbyortho}から
  \begin{align*}
    h(w)&=h(s_{\alpha_1}\cdots s_{\alpha_l})\\
    &=\hat t(s_{\alpha_1}\cdots s_{\alpha_l} \hat t^{-1}\\
    &=(\hat t(s_{\alpha_1} \hat t^{-1})\cdots (\hat t(s_{\alpha_l} \hat t^{-1})\\
    &=\hat s_{t(\alpha_1)}\cdots \hat s_{t(\alpha_l)} 
  \end{align*}
  となるので, $h(w)\in W(\Psi)$である.
  したがって, $h$は$W(\Phi)$から$W'$への写像であるが,
  $W(\Psi)=\Braket{R(\Psi)}_\ZZ$であるので,
  全射である.
  また $h(w)=h(u)$とすると,
  \begin{align*}
    \hat t w \hat t^{-1}&=\hat t u \hat t^{-1}\\
  \hat t^{-1}  \hat t w \hat t^{-1}\hat t&=\hat t^{-1}\hat t u \hat t^{-1}\hat t\\
    w=u
  \end{align*}
  となるので, 単射である.
  また$h(wu)=\hat t w u\hat t^{-1}=\hat t w \hat t^{-1}\hat t u \hat t^{-1}=h(w)h(u)$
  であるので, $W(\Phi)$と$W'$は同型である.
\end{proof}
...
$\Phi'$,
単純系を$\Delta'$とする.
$\alpha_0=\varepsilon_1$,
$\alpha_1=\varepsilon_2$である.
また,
...
\cref{ex:g2:simple}で考えた単純系を$\Delta'$とする.
$f(\alpha_0)=\alpha{1,2}$,
$f(\alpha_5)=\gamma{1}$とする.
このとき,
$t(\alpha_0)=\frac{1}{\sqrt 2}\alpha_{1,2}$,
$t(\alpha_5)=-\frac{1}{\sqrt 6}\alpha_{1,2}-\frac{1}{\sqrt 6}\alpha_{1,3}$
とすると,
$\Braket{t(\alpha_0),t(\alpha_5)}=\frac{1}{2\sqrt 3}\Braket{\alpha_{1,2},\gamma{1}}=-\frac{\sqrt 3}{2}$
である.
また,
$\Braket{\alpha_0,\alpha_5}=\cos(\frac{5\pi}{6})=-\frac{\sqrt{3}}{2}$
である.
また,
直接計算することで,
$\Set{t(\alpha)|\alpha\in\Phi}=\Set{\bar\alpha|\alpha\in \Phi'}$であることはわかる.
...
$m=2M$のとき,
$R_{M}=R_{-M}$であるが,
$\ee_\Delta(R_{M})=\ee_\Delta(R_{M})=m$がなりたつので,
この元が最長である.
$m=2M+1$のとき,
$P_{M}=P_{-M}$であるが,
$\ee_\Delta(R_{M})=\ee_\Delta(R_{M})=m$がなりたつので,
...
\begin{cor}
  $0<l< \frac{1}{2}\numof{\Phi}$に対し,
  $W_{l+1}=\Set{ws_\delta|\delta\in\Delta}\setminus W_{l-1}$.
\end{cor}
...
%\label{ex:a1n:inversion}
\cref{ex:a1n:simple}で考えたルート系$\Phi$と単純系$\Delta$について考える.
$W(\Phi)=\Set{\diag(a_1,\ldots,a_n)|a_i\in \ZZ^\times}$である.
$\diag(a_1,\ldots,a_n)\diag(b_1,\ldots,b_n)=\diag(a_1b_1,\ldots,a_nb_n)=\diag(b_1,\ldots,b_n)\diag(a_1,\ldots,a_n)$
であるので, $W(\Phi)$は可換群である.
