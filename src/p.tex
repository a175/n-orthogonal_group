...
\begin{lemma}
  ...
  \item $w(\delta\in\Phi^- \implies n(ws_\delta)=n(w)-1$.
    \label{item:buruhat:formula:-1}
  \item
    \label{item:buruhat:formula:r:1}
    $w(\delta)^{-1}\in\Phi^+ \implies n(s_\delta w)=n(w)+1$.
  \item
    \label{item:buruhat:formula:r:-1}
    $w(\delta)^{-1}\in\Phi^- \implies n(s_\delta w)=n(w)-1$.
  \end{enumerate}
...
\begin{cor}
  $w\in W$とする.
  $w=s_{\delta_1}\cdots s_{\delta_r}$, $(\delta\in\Delta)$
  とする.
  このとき,
  $n(w)\leq r$である.
  特に, $n(w)\leq \ell(w)$である.
\end{cor}
\begin{proof}
  \begin{align*}
...
\begin{lemma}
$\delta_r\in\Delta$に対し,
  $w=s_{\delta_1}\cdots s_{\delta_r}$
  とする.
  $n(w)<r$ならば,
  $\delta_i=s_{\alpha_{i+1}}\cdots s_{\alpha_{j-1}}(\alpha_j)$
  となる, $1\leq i\leq j \leq r$
  が存在する.
\end{lemma}
